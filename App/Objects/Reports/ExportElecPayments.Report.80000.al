...
{
    Caption = 'Export Electronic Payments (No Carrier)';
    ApplicationArea = All;
    ...
    dataset
    {
        dataitem("Gen. Journal Line"; "Gen. Journal Line")
        {
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                column(EntityPicture; DimensionValue.BssiPicture)
                {
                }
                column(PurchLoadNo; PurchLoadNo)
                {
                }
                column(EmailAddr; EmailAddr)
                {
                }
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    dataitem(PrintCompanyAddress; "Integer")
                    {
                        ...
                        begin
                            CompanyInfo.Get();
                            if PrintCompanyPicture then CompanyInfo.CalcFields(Picture);
                            if PrintCompanyAdd then begin
                                FormatAddress.Company(CompanyAddress, CompanyInfo);
                                if CompanyAddress[5] <> '' then CompanyAddress[5] := StrSubstNo(', %1', CompanyAddress[5]);
                            end;
                        end;
                    }
                    dataitem("Cust. Ledger Entry"; "Cust. Ledger Entry")
                    {
                        ...
                        begin
                            ...
                            if ("Pmt. Discount Date" >= "Gen. Journal Line"."Document Date") and ("Remaining Pmt. Disc. Possible" <> 0) and ((-ExportAmount - TotalAmountPaid) - "Remaining Pmt. Disc. Possible" >= -"Amount to Apply") then begin
                        ...
                        begin
                            ...
                            if BankAccountIs = BankAccountIs::Acnt then begin
                                if "Gen. Journal Line"."Bal. Account Type" <> "Gen. Journal Line"."Bal. Account Type"::Customer then CurrReport.Break();
                                SetRange("Customer No.", "Gen. Journal Line"."Bal. Account No.");
                            end
                            else begin
                                if "Gen. Journal Line"."Account Type" <> "Gen. Journal Line"."Account Type"::Customer then CurrReport.Break();
                    ...
                    dataitem("Vendor Ledger Entry"; "Vendor Ledger Entry")
                    {
                        ...
                        begin
                            ...
                            if "Gen. Journal Line"."Currency Code" = '' then begin
                                ...
                                if Currency.FindFirst() then
                                    TotalAmountPaid := TotalAmountPaid + Round(CurrExchRate.ApplnExchangeAmtFCYToFCY("Posting Date", "Currency Code", "Gen. Journal Line"."Currency Code", AmountPaid, ValidExchRate), Currency."Amount Rounding Precision")
                                else
                                    TotalAmountPaid := TotalAmountPaid + AmountPaid;
                            end
                            else
                            ...
                            IsSummary := true;
                            Clear(BuyFromAddr);
                            if ("Vendor Ledger Entry"."Document Type" = "Vendor Ledger Entry"."Document Type"::"Invoice") and PurchInvHeader.Get("Vendor Ledger Entry"."Document No.") then begin
                        ...
                        begin
                            if "Gen. Journal Line"."Applies-to ID" = '' then CurrReport.Break();
                            if BankAccountIs = BankAccountIs::Acnt then begin
                                if "Gen. Journal Line"."Bal. Account Type" <> "Gen. Journal Line"."Bal. Account Type"::Vendor then CurrReport.Break();
                                SetRange("Vendor No.", "Gen. Journal Line"."Bal. Account No.");
                            end
                            else begin
                                if "Gen. Journal Line"."Account Type" <> "Gen. Journal Line"."Account Type"::Vendor then CurrReport.Break();
                    ...
                    dataitem(CustomerInfo; "Integer")
                    {
                        ...
                        trigger OnPreDataItem()
                        begin
                            if (PayeeType <> PayeeType::Customer) or IsSummary then CurrReport.Break();
                        end;
                    }
                    dataitem(VendorInfo; "Integer")
                    {
                        ...
                        trigger OnPreDataItem()
                        begin
                            if (PayeeType <> PayeeType::Vendor) or IsSummary then CurrReport.Break();
                        end;
                    }
                    dataitem(Unapplied; "Integer")
                    {
                        ...
                        trigger OnPreDataItem()
                        begin
                            if TotalAmountPaid >= -ExportAmount then CurrReport.Break();
                        end;
                ...
                begin
                    ...
                    if Number = 1 then // Original
                        ...
                        CopyTxt := CopyLoopLbl;
                    if "Gen. Journal Line"."Applies-to Doc. No." = '' then Clear(TotalAmountPaid);
                end;
            ...
            begin
                ...
                if BankAccountIs = BankAccountIs::Acnt then begin
                    ...
                    if "Bal. Account Type" = "Bal. Account Type"::Vendor then begin
                        ...
                        Error(AccountTypeErr, FieldCaption("Bal. Account Type"), Customer.TableCaption(), Vendor.TableCaption());
                end
                else begin
                    ExportAmount := -"Amount (LCY)";
                    PayeeNo := "Account No.";
                    if "Account Type" = "Account Type"::Vendor then begin
                        ...
                        Error(AccountTypeErr, FieldCaption("Account Type"), Customer.TableCaption(), Vendor.TableCaption());
                end;
                DiscountTaken := 0;
                AmountPaid := 0;
                TotalAmountPaid := 0;
                ...
                if PayeeAddress[5] <> '' then PayeeAddress[5] := StrSubstNo(', %1', PayeeAddress[5]);
                ...
                if ("Gen. Journal Line"."Account Type" = "Gen. Journal Line"."Account Type"::Vendor) and (Vendor2.Get("Gen. Journal Line"."Account No.")) then begin
                    CustRepSelection.SetRange("Source Type", Database::Vendor);
                    CustRepSelection.SetRange("Source No.", Vendor2."No.");
    ...
    begin
        ...
        if GenJournalLine.FindFirst() then begin
            ...
            if not GenJournalTemplate."Force Doc. Balance" then if not Confirm(CannotVoidQst, true) then Error(UserCancelledErr);
        ...
        if PrintCompanyAdd then FormatAddress.Company(CompanyAddress, CompanyInformation);
    end;
    ...
    begin
        ...
        if GenJnlLine."Applies-to Doc. No." <> '' then begin
            ...
            if (VendLedgEntry."Pmt. Discount Date" >= GenJnlLine."Document Date") and (VendLedgEntry."Remaining Pmt. Disc. Possible" <> 0) and (-(ExportAmount + VendLedgEntry."Remaining Pmt. Disc. Possible") >= -VendLedgEntry.Amount) then begin
            ...
            else if -ExportAmount > -VendLedgEntry.Amount then
    ...
    begin
        ...
        if GenJnlLine."Applies-to Doc. No." <> '' then begin
            ...
            if (CustLedgEntry."Pmt. Discount Date" >= GenJnlLine."Document Date") and (CustLedgEntry."Remaining Pmt. Disc. Possible" <> 0) and (-(ExportAmount - CustLedgEntry."Remaining Pmt. Disc. Possible") >= -CustLedgEntry.Amount) then begin
            ...
            else if -ExportAmount > -CustLedgEntry.Amount then
